...
export function AuthScreen({ onSkip }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (error) throw error;
      // onAuthStateChange in page.tsx will handle the rest
    } catch (err: unknown) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#070d1a] px-5 py-10">
      ...
      {/* Card */}
      <div className="w-full max-w-sm rounded-[1.7rem] border border-white/8 bg-slate-950/80 p-6 shadow-[0_0_48px_rgba(0,0,0,0.5)] backdrop-blur-xl">
        {error && (
        ...
        {mode !== "reset" && (
          <>
            ...
            <button
              ...
              disabled={loading}
              className="flex w-full items-center justify-center gap-2.5 rounded-2xl border border-white/10 bg-slate-900 py-3.5 text-sm font-bold text-slate-200 transition-all hover:border-white/20 active:scale-95 disabled:opacity-50"
            >
